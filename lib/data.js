...
// The date the physical inventory count reflects (from the "AS AT" cell in
// the source Excel's PHY-Inventory-List sheet). Updated whenever a new file
// is merged in via Upload Inventory File.
export async function getInventoryAsOfDate() {
  await ensureSeeded();
  let setting = await prisma.appSetting.findUnique({ where: { id: 1 } });
  if (!setting) {
  ...
  return setting?.inventoryAsOfDate ? setting.inventoryAsOfDate.toISOString().slice(0, 10) : null;
}
