...
// Net Received vs Issued for Sand & Aggregate items, split by source location
// (Thilafushi / Mamigili) -- built from the bin-card history notes that were
// tagged with a "[Location]" prefix on import. This is a movement TOTAL, not
// a per-location current balance (there's no per-location opening figure to
...
export async function getLocationBreakdown() {
  ...
  for (const r of rows) {
    ...
    if (r.type === "GRN") result[loc].received += r.quantity;
    else result[loc].issued += r.quantity;
  }
  result.Thilafushi.received = round2(result.Thilafushi.received);
  result.Thilafushi.issued = round2(result.Thilafushi.issued);
  result.Mamigili.received = round2(result.Mamigili.received);
  result.Mamigili.issued = round2(result.Mamigili.issued);
  return result;
}
